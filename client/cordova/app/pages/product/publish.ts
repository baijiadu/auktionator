...
import {Mixins, Config, Async, QiniuUtil, Util} from '../../util/index';
import {SelectAuktionatorPage} from '../../pages/index';
...
export class ProductPublishPage {
  product: any;
  selectedAuktionator: any;
  uploadImageLimit: number;

  constructor(private nav:NavController, private userService:UserService, private productService:ProductService, private params:NavParams, private viewCtrl:ViewController) {
  ...
  onSubmit() {
    ...
    this.nav.present(loading).then(() => {
      let needUploadImages = uploadImages.filter((item) => !!item.file);
